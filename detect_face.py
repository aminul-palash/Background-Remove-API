...
def create_mtcnn(sess):
    with tf.compat.v1.variable_scope('pnet'):
        ...
        pnet = PNet({'data':data})
        i = dir_path + 'detection_models/det1.npy'
        pnet.load(dir_path + 'detection_models/det1.npy', sess)
        # pnet.load('./detection_models/det1.npy', sess)
    with tf.compat.v1.variable_scope('rnet'):
        data = tf.compat.v1.placeholder(tf.float32, (None,24,24,3), 'input')
        rnet = RNet({'data':data})
        rnet.load(dir_path + 'detection_models/det2.npy', sess)
    with tf.compat.v1.variable_scope('onet'):
        data = tf.compat.v1.placeholder(tf.float32, (None,48,48,3), 'input')
        onet = ONet({'data':data})
        onet.load(dir_path +  'detection_models/det3.npy', sess)
