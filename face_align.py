...
def rotation_detection_dlib(img, mode, show=False):
    ...
    predictor = dlib.shape_predictor('pretrained_model/shape_predictor_5_face_landmarks.dat')
...
def rotation_detection_opencv(img, mode, show=False):
    nose_cascade = cv2.CascadeClassifier('pretrained_model/haarcascade_mcs_nose.xml')
    eyes_cascade = cv2.CascadeClassifier('pretrained_model/haarcascade_eye.xml')
    fase_cascade = cv2.CascadeClassifier('pretrained_model/haarcascade_frontalface_default.xml')
...
def face_alignment(img_path):
    rotation_mode = 0 # 0 if Pillow rotation , 1 if opencv
    # img = load_img(img_path )
    img = img_path
    img = rotation_detection_dlib(img, rotation_mode,show=False)
    return img
